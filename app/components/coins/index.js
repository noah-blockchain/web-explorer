    ...
    <table className={`table__table table__table--${modificataion}`}>
      <thead className="table__header">
        <tr>
          ...
          <th
            className="table__cell"
            onClick={() => filter.setFilter('symbol')}
          >
      ...
      <tbody className="table__body">
        {data.map((item, i) => (
          <tr className="table__row" key={i}>
            ...
            <th className="table__cell">
              {' '}
              {new BigNumber(item.volume).toFormat(2)}
            </th>
            <th className="table__cell">
              {' '}
              {new BigNumber(item.reserveBalance).toFormat(2)}
            </th>
            <th className="table__cell table__link">
              <Link href={`/coins/${item.symbol}`}>
                <a className="link_theme_none">{item.name}</a>
              </Link>
            </th>
            <th className="table__cell table__link">
              <Link href={`/coins/${item.symbol}`}>
                <a className="link_theme_none">{item.symbol}</a>
              </Link>
            </th>
            <th className="table__cell">
              {new BigNumber(item.price).toFormat(2)}
            </th>
            <th className="table__cell">
              {new BigNumber(item.delegated).toFormat(2)}
            </th>
            <th className="table__cell">
              {new BigNumber(item.volume * item.price).toFormat(2)} NOAH
            </th>
